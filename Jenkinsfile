...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
				rmsg = sh returnStdout: true, script: "${toolbelt}/sfdx force:mdapi:deploy -c -u ${HUB_ORG}"
			}else{
			   rmsg = bat returnStdout: true, script: "\"${toolbelt}/sfdx\" force:mdapi:deploy -c manifest/. -u ${HUB_ORG}"
			}
